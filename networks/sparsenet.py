...
class Bottleneck(nn.Sequential):
    def __init__(self, nChannels, growthRate, dropRate=0.0):
        ...
        if dropRate > 0:
            # test inplace
            self.dropout1 = nn.Dropout3d(dropRate)
        ...
        if dropRate > 0:
            # test inplace
            self.dropout2 = nn.Dropout3d(dropRate)
...
class SingleLayer(nn.Sequential):
    def __init__(self, nChannels, growthRate, dropRate=0.0):
        ...
        self.bn1 = nn.BatchNorm3d(nChannels)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv3d(nChannels, growthRate, kernel_size=3, padding=1, bias=False)
        if dropRate > 0:
            # test inplace
            self.dropout1 = nn.Dropout3d(dropRate)
...
class Transition(nn.Sequential):
    def __init__(self, nChannels, nOutChannels, dropRate=0.0):
        ...
        self.bn1 = nn.BatchNorm3d(nChannels)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv3d(nChannels, nOutChannels, kernel_size=1, bias=False)
        if dropRate > 0.0:
            self.dropout1 = nn.Dropout3d(dropRate)
...
class DenseNet(nn.Module):
    def __init__(self,sample_size, sample_duration,  growthRate, depth, reduction, nClasses, bottleneck, layer_per_stage=None, grate_per_stage=None,
        ...
        self.features.add_module("norm_last", nn.BatchNorm3d(nChannels))
        ...
        self.classifier = nn.Sequential(OrderedDict([
            ("avgpool", nn.AvgPool3d(last_duration, last_size, last_size)),
