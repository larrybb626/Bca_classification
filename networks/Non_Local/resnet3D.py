...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, padding=(1, 1, 1), downsample=None):
        ...
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=(3, 3, 3), stride=stride, padding=padding, bias=False)
        self.bn2 = nn.BatchNorm3d(planes)
        # self.conv3 = nn.Conv3d(planes, planes * 4, kernel_size=(1, 1, 1), bias=False)
        self.conv3 = nn.Conv3d(planes, planes * 4, kernel_size=(3, 1, 1), padding=(1, 0, 0), bias=False)
...
class ResNet3D(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=400, non_local=False):
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, padding=(3, 1, 1))
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, padding=(3, 1, 1), non_local=non_local)

        # output: 2048 x 4 x 7 x 7
        # self.layer4 = self._make_layer(block, 512, layers[3], stride=2, padding=(2, 1, 1))
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, padding=(3, 1, 1))
    ...
    def _make_layer(self, block, planes, blocks, stride=1, padding=(1, 1, 1), d_padding=(2, 0, 0), non_local=False):
...
if __name__=='__main__':
    ...
    img = Variable(torch.randn(1, 1, 32, 224, 224))
