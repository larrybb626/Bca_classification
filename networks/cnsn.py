...
def calc_ins_mean_std(x, eps=1e-5):
    ...
    assert (len(size) == 4)
    N, C = size[:2]
    var = x.contiguous().view(N, C, -1).var(dim=1) + eps
...
if __name__=='__main__':
    input = torch.randn((48,3,128,128))
    mySelfNorm = SelfNorm(chan_num=4096,is_two=True)
    myCrossNorm = CrossNorm(crop=0.5,beta=0.5)
    cnsn = CNSN(myCrossNorm,mySelfNorm)
    output = cnsn(input)
    print(output.shape)
