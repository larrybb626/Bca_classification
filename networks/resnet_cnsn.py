...
class BottleneckCustom(nn.Module):
    ...
    def __init__(self, inplanes, planes, pos, beta, crop, cnsn_type,
        ...
        if cnsn_type is not None:  # for CrossNorm in image space, cnsn_type is None
...
class ResNet(nn.Module):
    ...
    def _make_layer_custom(self, block, planes, blocks, pos, beta,
                           crop, cnsn_type, stride=1, dilate=False):
...
def resnet50(config):
    ...
    model = ResNet([3, 4, 6, 3],
                   ...
                   beta=config.beta, crop=config.crop,
