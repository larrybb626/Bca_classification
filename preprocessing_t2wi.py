...
def non_local_means_denoising(o_data, patch_size=5, patch_distance=3, h=0.1):
    print('#Non-Local Means Denoising...')
    denoised_data = []
    for i in range(o_data.shape[2]):
        slice_denoised = denoise_nl_means(o_data[:, :, i], patch_size=patch_size, patch_distance=patch_distance, h=h)
        denoised_data.append(slice_denoised)
    denoised_data = np.array(denoised_data).transpose(1, 2, 0)
    print("--Denoised size:", denoised_data.shape)
    return denoised_data

if __name__ == "__main__":

    """Setting the path"""
    roi_root = r'Y:\newnas_1\huyilan202124\LvJieGeng-BCa\SUNYATSEN-H\ZhongShanErYuan\NII\T2WI NII'
    data_root = r'Y:\newnas_1\huyilan202124\LvJieGeng-BCa\SUNYATSEN-H\ZhongShanErYuan\T2WI_nii_img'
    save_root = r"Y:\newnas_1\huyilan202124\LvJieGeng-BCa\SUNYATSEN-H\ZhongShanErYuan\h5"
    h5_save_path = os.path.join(save_root, 't2wi_h5')
    ...
    for filename in roi_list:
        ...
        # 使用布尔索引将大于 1500 的数置为 1500
        img_arr[img_arr > 1500] = 1500

        # 添加非局部均值滤波步骤
        img_arr = non_local_means_denoising(img_arr)
